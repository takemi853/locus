...
git clone git@github.com:takemi853/my-knowledge-base.git ~/Projects/locus-project/locus
cd ~/Projects/locus-project/locus
uv sync

# データリポジトリ（daily/ / knowledge/ を格納）
# private 環境:
git clone git@github.com:takemi853/locus-private.git ~/Projects/locus-project/locus-private
# work 環境（例）:
# git clone git@github.com:takemi853/knowledge-work.git ~/Projects/locus-project/knowledge-work
```
...
```yaml
data_dir: ~/Projects/locus-project/locus-private   # private 環境
# data_dir: ~/Projects/locus-project/knowledge-work    # work 環境
```
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "cd /Users/yourname/Projects/locus-project/locus && uv run python hooks/session-start.py",
        "timeout": 15
      }]
    }],
    "PreCompact": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "cd /Users/yourname/Projects/locus-project/locus && uv run python hooks/pre-compact.py",
        "timeout": 10
      }]
    }],
    "SessionEnd": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "cd /Users/yourname/Projects/locus-project/locus && uv run python hooks/session-end.py",
        "timeout": 10
      }]
    }]
  }
...
# 以下を追記（5分ごとに実行）
*/5 * * * * cd ~/Projects/locus-project/locus && uv run python scripts/flush_periodic.py >> /tmp/claude-kb-flush.log 2>&1
...
```bash
cd ~/Projects/locus-project
git clone https://github.com/jackyzha0/quartz.git knowledge-ui
cd knowledge-ui
npm ci
rm -rf content
ln -s ~/Projects/locus-project/locus-private/knowledge content   # data_dir に合わせる
...
```bash
# データリポジトリを更新（1日1回程度）
cd ~/Projects/locus-project/locus-private
git add .
git commit -m "ナレッジ更新"
git push
...
data_dir: ~/Projects/locus-project/locus-private  # データリポジトリの場所
...
# settings.yaml
environment: private
data_dir: ~/Projects/locus-project/locus-private
llm:
  backend: claude_code
  model: claude-sonnet-4-6
```
...
# settings.yaml
environment: work
data_dir: ~/Projects/locus-project/knowledge-work
llm:
  backend: vertex_ai
  model: claude-sonnet-4-6
  vertex_ai:
...
git clone git@github.com:yourname/knowledge-work.git ~/Projects/locus-project/knowledge-work

# または既存データなしで新規作成
mkdir -p ~/Projects/locus-project/knowledge-work && cd ~/Projects/locus-project/knowledge-work && git init
