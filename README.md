...
| リポジトリ | 内容 | 共有 |
|---|---|---|
| `claude-memory-compiler`（本リポジトリ） | スクリプト・hooks・設定 | private / work 共通 |
| `knowledge-private`（データリポジトリ） | `daily/` `knowledge/` | 環境ごとに独立 |
...
# private 環境:
git clone git@github.com:takemi853/knowledge-private.git ~/Projects/knowledge-private
# work 環境（例）:
...
```yaml
data_dir: ~/Projects/knowledge-private   # private 環境
...
# 以下を追記（5分ごとに実行）
*/5 * * * * cd ~/Projects/app/claude-memory-compiler && uv run python scripts/flush_periodic.py >> /tmp/claude-kb-flush.log 2>&1
...
npm ci
rm -rf content
ln -s ~/Projects/knowledge-private/knowledge content   # data_dir に合わせる
...
```
claude-memory-compiler/          ← システムリポジトリ（スクリプト）
├── hooks/
...
knowledge-private/               ← データリポジトリ（data_dir で指定）
├── daily/                    会話の要約ログ（自動生成）
└── knowledge/
...
data_dir: ~/Projects/knowledge-private  # データリポジトリの場所
